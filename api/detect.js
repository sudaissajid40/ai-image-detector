        ...
        try {
          ...
          if (geminiRes.ok) {
            ...
            if (textResponse) {
              ...
              geminiError = null; // Clear any previous errors on success
            ...
            console.error(`[detect] Gemini API error status for ${modelName}:`, geminiRes.status, geminiError);
          }
        } catch (geminiErr) {
          geminiError = geminiErr.message || String(geminiErr);
          console.error(`[detect] Gemini integration error for ${modelName}:`, geminiErr);
        }
      }

      // If both models failed and it's a 503/404, collect available models diagnostic
      if (!geminiData && geminiError) {
        let extraDebug = '';
        try {
